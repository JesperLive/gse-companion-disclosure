...
**Addon 3.3.24-2** removed the `GSE.Patron` flag entirely, 17 references down to zero, and moved the gated features into a patron-only `GSE_QoL` module. For three of the five features that is a real change. For raw edit and multi-window it is not: the code is still compiled into the free build and withheld by a nil function.
...
- "A paid build with additional for-pay features exists at all", which is what Blizzard's UI Add-On Development Policy point 1 speaks to — not addressed. Three extra files still deliver four extra features to payers.
...
`QoL.lua` is 473 lines of add-on programming code that is not freely accessible to or viewable by the general public — it sits behind a Patreon role lock. Nothing in it is obfuscated, so the "hidden or obfuscated" half of that sentence is not the issue; the "freely accessible to and viewable by the general public" half is. Under the old `GSE.Patron` arrangement all the code shipped to everyone and only a flag was withheld, which sat awkwardly with point 1 but not with point 2. Moving the code into a role-locked module is a cleaner answer to point 1 and a worse one for point 2. I flag that as a reading of the text, not a ruling — Blizzard decides what its policy means, not me.
